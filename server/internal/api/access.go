...
// from auth.default_role on first contact (member unless configured; none
...
// "" means not enrolled: access comes from explicit per-repo grants only.
func (s *Server) deployRole(u *store.User) (string, error) {
	...
	if role == "none" {
		return "", nil // restricted deployment: access comes from grants only
	}
