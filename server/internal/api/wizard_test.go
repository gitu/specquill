...
func TestWizardInterviewStructuresTheTurn(t *testing.T) {
	reply := `{"reply":"Retention is already specified at 5 years in specs/retention.md.",
		"questions":[{"question":"Does the new rule replace the 5-year window?","options":["Replaces it","Runs alongside"]}],
...
func TestWizardComposeNormalizesAgainstTheOutline(t *testing.T) {
	// markdown, not JSON: out of order, re-cased, one requested block missing,
	// one extra — and a body carrying the characters that broke the envelope
	reply := "# Seven-year retention\n\n" +
		"## edge cases\n\nLegal hold.\n\n" +
		"## Overview\n\nRecords live seven years, quoting \"Booking date\".\n\n" +
		"## Assumptions\n\nExisting records are re-stamped.\n"
	h, _ := testWizardServer(t, fakeTurn{content: reply})
	cookie := login(t, h)
	...
	// the quotes that used to break the JSON envelope survive intact
	if !strings.Contains(sections[0].(map[string]any)["content"].(string), `"Booking date"`) {
		t.Fatalf("quotes lost through the stage: %+v", sections[0])
	}
}

// A compose reply with no `## ` headings is a real failure, not an empty draft
// silently handed to the author.
func TestWizardComposeRejectsAReplyWithNoSections(t *testing.T) {
	h, _ := testWizardServer(t, fakeTurn{content: "I could not write this document."})
	cookie := login(t, h)
	_, _, errMsg := wizardPost(t, h, cookie, "/api/repos/w/speccy/compose", map[string]any{
		"intent": "x", "family": "spec", "sections": []string{"Overview"},
	})
	if errMsg == "" {
		t.Fatal("a section-less reply must fail loudly")
	}
}
...
func TestWizardSectionTakesNoteAndBody(t *testing.T) {
	h, fake := testWizardServer(t, fakeTurn{content: "NOTE: cut two sentences\nTightened, with a \"quote\" and a \\ backslash."})
	cookie := login(t, h)
	...
	if result["note"] != "cut two sentences" {
		t.Fatalf("note = %v", result["note"])
	}
	if !strings.Contains(result["content"].(string), `"quote"`) {
		t.Fatalf("content = %v", result["content"])
	}
	// prose stages need no repair round: any text is a valid answer
	if fake.seen != 1 {
		t.Fatalf("prose stage should not retry, provider saw %d calls", fake.seen)
	}
}

// The JSON stages keep their corrective re-ask.
func TestWizardRetriesWhenAJSONStageWrapsItInProse(t *testing.T) {
	h, fake := testWizardServer(t,
		fakeTurn{content: "Sure! Here is my analysis, which is definitely not JSON."},
		fakeTurn{content: `{"matches":[],"recommendation":"new"}`},
	)
	cookie := login(t, h)

	result, _, errMsg := wizardPost(t, h, cookie, "/api/repos/w/speccy/related", map[string]any{
		"intent": "x", "family": "spec",
	})
	if errMsg != "" {
		t.Fatalf("related errored: %s", errMsg)
	}
...
// The prompt asks for {question, options} objects, but smaller models revert
// to bare strings. Both shapes must survive, and an over-long option list is
// capped rather than pushed at the author whole.
func TestWizardInterviewAcceptsBothQuestionShapes(t *testing.T) {
	reply := `{"reply":"ok","readyToDraft":false,"rubric":[],"questions":[
		"a bare string question",
		{"question":"an object question","options":["one","two","three","four","five","six","seven"]},
		{"question":"   "},
	...
	result, _, errMsg := wizardPost(t, h, cookie, "/api/repos/w/speccy/interview", map[string]any{
		"intent": "x", "family": "spec",
		"messages": []map[string]string{{"role": "user", "content": "prior answer"}},
	})
	if errMsg != "" {
		t.Fatalf("interview errored: %s", errMsg)
	}
	qs, _ := result["questions"].([]any)
	if len(qs) != 3 {
		t.Fatalf("expected the blank question dropped, got %d: %+v", len(qs), qs)
	}
	first := qs[0].(map[string]any)
	if first["question"] != "a bare string question" {
		t.Fatalf("bare string not coerced: %+v", first)
	}
	if _, ok := first["options"]; ok {
		t.Fatalf("a string question must not invent options: %+v", first)
	}
	second := qs[1].(map[string]any)
	opts, _ := second["options"].([]any)
	if len(opts) != 5 {
