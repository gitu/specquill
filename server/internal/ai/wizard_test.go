...
// The compose stage answers in markdown because section bodies are prose:
// quotes, backslashes and table pipes inside a JSON string are what actually
// broke real drafts (gpt-5.5 emitted "invalid character 'B'/'\\' looking for
// beginning of object key string"). Markdown escapes nothing, so these parse.
func TestParseDraftSurvivesWhatBrokeTheJSONEnvelope(t *testing.T) {
	reply := "# Seven-year retention\n\n" +
		"## Overview\n\nRecords quoting \"Booking date\" are kept.\n\n" +
		"## Behaviour\n\n| a \\| b | c |\n|---|---|\n\n```go\nx := \"y\"\n```\n\n" +
		"## Edge cases\n\nA path C:\\Users\\x and a lone \\ backslash.\n"
	title, sections := ParseDraft(reply)
	if title != "Seven-year retention" {
		t.Fatalf("title = %q", title)
	}
	if len(sections) != 3 {
		t.Fatalf("got %d sections: %+v", len(sections), sections)
	}
	if !strings.Contains(sections[0].Content, `"Booking date"`) {
		t.Fatalf("quotes lost: %q", sections[0].Content)
	}
	if !strings.Contains(sections[1].Content, "```go") || !strings.Contains(sections[1].Content, `\|`) {
		t.Fatalf("fenced code or escaped pipe lost: %q", sections[1].Content)
	}
	if !strings.Contains(sections[2].Content, `C:\Users\x`) {
		t.Fatalf("backslashes lost: %q", sections[2].Content)
	}
}

func TestParseDraftTolerance(t *testing.T) {
	// a whole-reply fence, and prose before the first heading
	title, sections := ParseDraft("```markdown\nchatter\n# T\n\n## A\n\nbody\n```")
	if title != "T" || len(sections) != 1 || sections[0].Content != "body" {
		t.Fatalf("fenced reply: title=%q sections=%+v", title, sections)
	}
	// no H1 at all is fine — the client falls back to its own title
	title, sections = ParseDraft("## A\n\nbody")
	if title != "" || len(sections) != 1 {
		t.Fatalf("headless reply: title=%q sections=%+v", title, sections)
	}
}

func TestParseSectionReply(t *testing.T) {
	content, note := ParseSectionReply("NOTE: cut two sentences\nThe tightened body.\nSecond line.")
	if note != "cut two sentences" || content != "The tightened body.\nSecond line." {
		t.Fatalf("note=%q content=%q", note, content)
	}
	// no note line: everything is body
	content, note = ParseSectionReply("Just the body.")
	if note != "" || content != "Just the body." {
		t.Fatalf("note=%q content=%q", note, content)
	}
	// a body that merely STARTS with the word keeps its content — the line is
	// only consumed when something follows it
	content, note = ParseSectionReply("NOTE: this is the whole body")
	if content != "NOTE: this is the whole body" || note != "" {
		t.Fatalf("swallowed the body: note=%q content=%q", note, content)
	}
}
