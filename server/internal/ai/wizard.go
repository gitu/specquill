...
2. ASK THE SMALLEST SET of questions that would actually change the draft. Never
   re-ask something the transcript already answers. Prefer concrete choices over
   open prompts.
...

{
  ...
  "questions": ["one short, specific question", "another"],
...
{
  "title": "short imperative document title",
  "sections": [{"name": "<exactly one of the requested names>", "content": "markdown body"}]
}`
}
...
Return the rewritten BODY only — no heading, no frontmatter — plus one short
line saying what changed.

# Reply format
Reply with ONLY a JSON object, no prose around it:

{"content": "the rewritten section body, markdown", "note": "one line on what changed"}`
}
