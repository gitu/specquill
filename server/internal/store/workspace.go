...
func (s *Store) WorkspaceBranch(repo string, userID int64) (string, error) {
	var branch string
	err := s.queryRow("SELECT branch FROM workspace_branches WHERE repo = ? AND user_id = ?", repo, userID).Scan(&branch)
...
func (s *Store) ClaimWorkspaceBranch(repo, branch string, userID int64) error {
	_, err := s.exec("INSERT INTO workspace_branches (repo, user_id, branch) VALUES (?, ?, ?)", repo, userID, branch)
	return err
}
...
func (s *Store) WorkspaceOwner(repo, branch string) (int64, error) {
	var id int64
	err := s.queryRow("SELECT user_id FROM workspace_branches WHERE repo = ? AND branch = ?", repo, branch).Scan(&id)
	if errors.Is(err, sql.ErrNoRows) {
