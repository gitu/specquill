...
test('the wizard walks intent → interview → draft → editor', async ({ page, request }) => {
  ...
  // each question offers concrete options; picking one is enough to answer it
  const questions = page.getByTestId('wizard-questions');
  await questions.getByRole('button', { name: 'Replaces it' }).click();
  await expect(questions.getByRole('button', { name: 'Replaces it' })).toHaveAttribute('aria-pressed', 'true');
  // the second question has no options — free text only
  await questions.getByPlaceholder('your answer…').fill('All trade records.');
  await expect(page.getByTestId('wizard-send')).toContainText('Send 2 answers');
  ...
  // escape EVERY regex metacharacter, not just the dot: the path is
  // interpolated into a RegExp, and a partial escape is the bug CodeQL flags
  const rePath = path.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  await expect(page).toHaveURL(new RegExp(`/editor/${rePath}`), { timeout: 20_000 });
...
test('nothing is written to the worktree until the author accepts', async ({ page, request }) => {
  ...
  await page.getByRole('button', { name: 'Skip — just draft it' }).click();
  await page.getByTestId('wizard-draft').click();
  await expect(page.getByTestId('wizard-sections')).toBeVisible({ timeout: 20_000 });
