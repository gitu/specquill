...
describe('wizard store', () => {
  ...
  it('coerces questions persisted before options existed', () => {
    // a wizard left open across the change would otherwise render undefined
    localStorage.setItem('specquill-wizard:old', JSON.stringify({
      stage: 'interview', questions: ['a bare string', { question: 'an object', options: ['x'] }, '  ', null],
    }));
    updateWizard('old', {});
    expect(read('old').questions).toEqual([
      { question: 'a bare string' },
      { question: 'an object', options: ['x'] },
    ]);
  });
